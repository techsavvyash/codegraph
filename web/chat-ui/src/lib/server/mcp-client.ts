...
// Resolve binary from CODEGRAPH_MCP_BIN if set, else fall back to the path
// relative to CWD (web/chat-ui when running `pnpm dev`).
// dirname(MCP_BINARY) = repo/bin/ — so the spawned process sees "../.env" = repo root .env
const MCP_BINARY = process.env.CODEGRAPH_MCP_BIN
  ? resolve(process.env.CODEGRAPH_MCP_BIN)
  : resolve(process.cwd(), '../../bin/codegraph-mcp')
